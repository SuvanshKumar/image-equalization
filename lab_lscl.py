...
lab_img = cv2.cvtColor(inputImage, cv2.COLOR_BGR2Lab)
window_height = H2 - H1 + 1
window_width = W2 - W1 + 1
window = lab_img[H1:H2+1, W1:W2+1].copy()
L_array = window[:,:,0]
a = np.min(L_array)
b = np.max(L_array)
A = 0
...
for i in range(window_height):
    for j in range(window_width):
        window[i, j, 0] = round((window[i, j, 0]-a)*(B-A)/(b-a) + A)
lab_img[H1:H2+1, W1:W2+1] = window
outputImage = cv2.cvtColor(lab_img, cv2.COLOR_Lab2BGR)
